    ...
    const completedTender = useQuery<Tender>({
        ...
        queryFn: () => {
            return Api.get(`przetargi/zakonczone/${id}`)
                .then((res) => res.data)
                .catch(() => {
                    setTenderError("Błąd w czasie pobierania danych odnośnie przetargu")
                })
        },
        enabled: true,
    })
    ...
    const offers = useQuery<Offer[]>({
        ...
        queryFn: () => {
            return Api.get(`oferty/${id}`)
                ...
                .catch(() => {
                    setOfferError("Błąd w czasie dodawania nowej oferty")
                })
        },
        enabled: true,
    })
                            ...
                            {completedTender.isSuccess ?
                                <>
                                    ...
                                    <div
                                        ...
                                        <div>
                                            {offerError ? (
                                                <p className="text-red-600 font-bold text-center">{offerError}</p>
                                            ) : (
                                                <>
                                                    {offers.data?.length > 0 ? (
                                                        <>
                                                            ...
                                                            <ul role="list"
                                                                className="w-full p-6 divide-y divide-slate-200">
                                                                {offers.data?.map((offer) => (
                                                                    <li className="first:pt-0 last:pb-0 py-4"
                                                                        key={offer.id}>
                                                        ...
                                                        <p className="text-center font-medium text-red-600">Przetarg
                                                            zakończony bez rozstrzygnięcia</p>
                                                    )}
                                                </>
                                            )}
                                        </div>
